...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the cwcc chaincode on the channel"
...
function networkUp() {
  ...
  if [ ! -d "organizations/peerOrganizations" ]; then
    echo "generate artifacts if they don't exist"
    createOrgs
    createConsortium
  fi
  ...
  echo "COMPOSE_FILES $COMPOSE_FILES $IMAGETAG"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt cwcc.tar.gz cwcc
